...
import { getJournal } from "../contentRegistry";
import EssayCard from "../components/EssayCard"; // Note: EssayCard is highly generic and fits both beautifully!

const Journal = () => {
  const [selectedCategory, setSelectedCategory] = useState("All");

  const journalEntries = getJournal();
  const categories = ["All", ...new Set(journalEntries.map(j => j.category))];

  const filteredEntries = selectedCategory === "All"
    ? journalEntries
    : journalEntries.filter(j => j.category === selectedCategory);

  const featuredEntry = journalEntries.find(j => j.featured);
    ...
    <div className="section-pad">
      <div className="container" style={{ maxWidth: "var(--reading-max)" }}>
        ...
        <div style={{ marginBottom: "var(--space-8)", textAlign: "center" }}>
          <h1 style={{ fontSize: "var(--fs-display-lg)", marginBottom: "var(--space-4)" }}>Reflections</h1>
          <p style={{ color: "var(--text-secondary)", fontSize: "var(--fs-body-lg)" }}>
            Personal reflections, mindset shifts, fitness disciplines, and life observations.
          </p>
        </div>
        ...
        {/* FEATURED ENTRY */}
        {selectedCategory === "All" && featuredEntry && (
          <div
          ...
          >
            ...
            <div style={{ display: "flex", gap: "24px", color: "var(--text-muted)", fontSize: "var(--fs-small)", fontFamily: "var(--font-mono)", marginBottom: "var(--space-5)" }}>
              <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><Tag size={12} /> {featuredEntry.category}</span>
              <span style={{ display: "flex", alignItems: "center", gap: "6px" }}><Clock size={12} /> {featuredEntry.readTime}</span>
            </div>
